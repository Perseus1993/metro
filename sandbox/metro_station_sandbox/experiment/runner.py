...
from ..visual_demo.mesa_export import mesa_frames_to_visual_tracks
from .diagnosis import TrajectoryReport, diagnose_tracks
from .report import write_experiment_report


DEFAULT_OUTPUT_DIR = Path("output") / "metro_experiment"
...
def build_cases(
    *,
    ...
    for design_label, design in designs:
        for entry_count, exit_count, seed in product(entries, exits, seeds):
            case_id = _case_id(design_label, entry_count, exit_count, seed)
            cases.append(
                ExperimentCase(
                    case_id=case_id,
                    design=design,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        write_experiment_report(
            results,
            args.output,
            replay_asset_dir=VISUAL_DEMO_ROOT / "assets" / "experiment_tracks",
...
def _case_id(design_label: str, entry_count: int, exit_count: int, seed: int) -> str:
...
def _print_summary(results: Sequence[CaseResult], output_dir: Path) -> None:
    ...
    for result in results:
        if result.tracks_payload is None:
            continue
        replay_file = f"assets/experiment_tracks/{result.case.case_id}_tracks.js"
        print(
            "[EXPERIMENT] animation="
            f"{VISUAL_DEMO_ROOT / 'animation_demo.html'}?file={replay_file}"
        )
