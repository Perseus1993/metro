...
from metro_station_testkit.goal_gate_physical_probe import (  # noqa: E402
    GOAL_GATE_COMPONENT_PROBE,
)


DEFAULT_OUTPUT_DIR = ROOT / "output" / "goal_physical_probe"
DEFAULT_JSON_OUT = DEFAULT_OUTPUT_DIR / "goal_physical_probe.json"
DEFAULT_MARKDOWN_OUT = DEFAULT_OUTPUT_DIR / "goal_physical_probe.md"


def build_report(*, seed: int = 42, max_seconds: float = 50.0) -> dict[str, Any]:
    return GOAL_GATE_COMPONENT_PROBE.run(seed=seed, max_seconds=max_seconds)


def render_markdown(report: dict[str, Any]) -> str:
    summary = report["summary"]
    lines = [
        "# 单旅客进站闸机 Graph × JuPedSim 物理探针",
        "",
        f"- 状态：`{summary['status']}`",
...
def _scenario_markdown(scenario: dict[str, Any]) -> list[str]:
    lines = [
        ...
        "",
        "| # | t | 事件 | 物理位置 | Graph之前 | Graph之后 | 设施 | 阻挡人数 | 命令 |",
        "|---:|---:|---|---|---|---|---|---:|---|",
    ]
    for trace in scenario["traces"]:
        ...
        lines.append(
            "| {index} | {time:g} | `{event}` | `{position}` | `{before}` | `{after}` | "
            "`{facility}` | {blockers} | `{commands}` |".format(
                index=trace["index"],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Goal Graph with a JuPedSim gate micro scene")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.quiet:
        print(
            f"[GOAL-PHYSICAL] status={report['summary']['status']} "
            f"scenarios={report['summary']['passed_scenarios']}/"
            f"{report['summary']['scenario_count']}"
        )
        print(f"[GOAL-PHYSICAL] json={args.json_out.resolve()}")
        print(f"[GOAL-PHYSICAL] markdown={args.markdown_out.resolve()}")
