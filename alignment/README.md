...
- observed v5 绑定 canonical Parquet、canonical meta 的相对路径、大小和 SHA-256；计算结束前再次复核输入与分析代码均未变化。simulation v5 从已哈希 raw snapshot 精确重建 canonical、provenance、metrics、support 和 summary；comparison v5 由当前 observed、simulation 与可信 SceneConfig 确定性重建；report v5 再由 comparison 确定性重建。任一 wrapper 版本或内容被改均失败。
- `platform_boarding` 内部几何仍为 bbox proxy；当前速度指标也只是非发布资格的代理。因此即使数值过线，报告仍必须是 `candidate_not_validated`，不能声称期望速度已标定。
- alignment 当前全量回归为 Ruff 通过、`176 passed`，其中包含新 Python 子进程执行 runner `--list-scenes` 的 clean-process smoke。该反例曾捕获共享 Metro 的循环导入，上游随后修复；alignment 没有用预加载顺序绕过。
- 当前仍没有可发布的 10 分钟 simulation v5。正式路径会在 `build_model` 前执行设计文档级源区预检：共享净距 0.396 m、峰值同 tick 下车批次 4、实际候选窗口 67；其中 60 个候选在 boarding holding area 内、64 个在其净距缓冲区内、4 个与门轴通道冲突。Metro core 现已为同一窗口物化 `alighting_source:queue_platform_edge_a_down` 容量证书，并在编译期稳定复现 `capacity.coactive_slot_conflict`。最近一次 artifact 因而标记 `runtime_status=not_started`、`scientific_status=model_invalid`，绝不解释为 `capacity_exceeded`；旧 v2 bundle 未被覆盖。
...
- 剩余通用修复属于 Metro core：train-specific exchange manifest、上/下车共用的 PTI 控制器、下车优先或有依据的混合策略、共享通道预约，以及有界 deadlock/hold。修复前 Step 5 失败、Step 6 旧 comparison 过期、Step 7 pending，implementation/release 均为 `hold`。
