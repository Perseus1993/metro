...
$replay = Join-Path $outputDirectory 'clearance_100_complete.replay.json'
$tracks = Join-Path $outputDirectory 'clearance_100_complete_tracks.js'
...
if (-not $audit.cleared -or $audit.total_agents -ne 100 -or $audit.remaining_agents -ne 0) {
    throw "Replay did not satisfy 100-person clearance acceptance: $($audit | ConvertTo-Json -Compress)"
}
...
Write-Output "Verified complete 100-person clearance replay: $replay"
