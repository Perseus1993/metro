...
namespace MetroReplay.Tests
{
    public sealed class PromotionalVideoCaptureTests
    {
        ...
        [Test]
        public void PortraitPreset_UsesNativeNineBySixteenRecorderResolution()
        {
            PromotionalVideoCapture.SetActive(true, true);

            Assert.That(PromotionalVideoCapture.IsPortrait, Is.True);
            Assert.That(PromotionalVideoCapture.OutputWidth, Is.EqualTo(1080));
            Assert.That(PromotionalVideoCapture.OutputHeight, Is.EqualTo(1920));
        }
