...
namespace MetroReplay.Presentation
{
    ...
    public static class OpeningCameraStorySampler
    {
        ...
        public const float PortraitOverviewArrivalDuration = 1.2f;
        public const float PortraitOverviewArrivalEnd = TrainHoldEnd + PortraitOverviewArrivalDuration;
        public const float PortraitPanoramaScanDuration = 6.8f;
        public const float PortraitDuration = PortraitOverviewArrivalEnd + PortraitPanoramaScanDuration;
        public const float PortraitBillboardDistance = 8.4f;
        public const float PortraitNoticeDistance = 13.2f;
        public const float PortraitWideShotDistanceMultiplier = 1.85f;
        ...
        public static OpeningCameraStorySample SamplePortrait(
            float elapsedSeconds,
            Vector3 artworkTarget,
            Vector3 billboardTarget,
            float matchCutDistance,
            CameraOrbitView exhibitionDestination,
            CameraOrbitView trainDestination,
            CameraOrbitView panoramaStart,
            CameraOrbitView panoramaEnd)
        {
            if (elapsedSeconds <= TrainHoldEnd)
            {
                return SampleThroughTrain(
                    elapsedSeconds,
                    artworkTarget,
                    billboardTarget,
                    matchCutDistance,
                    ...
                    trainDestination,
                    Mathf.Max(matchCutDistance, PortraitBillboardDistance),
                    PortraitNoticeDistance);
            }

            var elapsed = Mathf.Clamp(elapsedSeconds, TrainHoldEnd, PortraitDuration);
            if (elapsed <= PortraitOverviewArrivalEnd)
            {
                var arrivalProgress = SmootherStep(Mathf.InverseLerp(
                    TrainHoldEnd,
                    PortraitOverviewArrivalEnd,
                    elapsed));
                return new OpeningCameraStorySample(
                    Interpolate(trainDestination, panoramaStart, arrivalProgress),
                    OpeningCameraStoryBeat.Overview);
            }

            var scanProgress = SmootherStep(Mathf.InverseLerp(
                PortraitOverviewArrivalEnd,
                PortraitDuration,
                elapsed));
            return new OpeningCameraStorySample(
                Interpolate(panoramaStart, panoramaEnd, scanProgress),
                elapsed >= PortraitDuration
                    ? OpeningCameraStoryBeat.Complete
                    : OpeningCameraStoryBeat.Overview);
        }

        public static void CreatePortraitPanoramaScanViews(
            Bounds stationBounds,
            out CameraOrbitView panoramaStart,
            out CameraOrbitView panoramaEnd)
        {
            var targetY = stationBounds.center.y
                          - Mathf.Clamp(stationBounds.extents.y * 0.12f, 0.4f, 1.2f);
            var halfScan = stationBounds.extents.x * 0.72f;
            var distance = Mathf.Clamp(
                Mathf.Max(stationBounds.size.y * 2.8f, stationBounds.size.z * 0.78f),
                26f,
                46f);
            var center = new Vector3(stationBounds.center.x, targetY, stationBounds.center.z);
            panoramaStart = new CameraOrbitView(
                center - Vector3.right * halfScan,
                distance,
                152f,
                4.5f);
            panoramaEnd = new CameraOrbitView(
                center + Vector3.right * halfScan,
                distance,
                152f,
                4.5f);
        ...
        private static OpeningCameraStorySample SampleThroughTrain(
            float elapsedSeconds,
            Vector3 artworkTarget,
            Vector3 billboardTarget,
            float matchCutDistance,
        ...
        {
            var elapsed = Mathf.Clamp(elapsedSeconds, 0f, TrainHoldEnd);
            var artworkView = new CameraOrbitView(
                artworkTarget,
                Mathf.Max(0.35f, matchCutDistance),
                180f,
            ...
            if (elapsed < MatchCutHoldEnd)
                return new OpeningCameraStorySample(artworkView, OpeningCameraStoryBeat.Billboard);
            if (elapsed < BillboardHoldEnd)
            {
                var billboardProgress = SmootherStep(Mathf.InverseLerp(
                    MatchCutHoldEnd,
                    BillboardHoldEnd,
                    elapsed));
                return new OpeningCameraStorySample(
                    Interpolate(artworkView, billboardView, billboardProgress),
                    OpeningCameraStoryBeat.Billboard);
            }
            if (elapsed < DepartureNoticeEnd)
            {
                var noticeProgress = SmootherStep(Mathf.InverseLerp(
                    BillboardHoldEnd,
                    DepartureNoticeEnd,
                    elapsed));
                return new OpeningCameraStorySample(
                    Interpolate(billboardView, noticeView, noticeProgress),
                    OpeningCameraStoryBeat.DepartureNotice);
            }
            if (elapsed <= ConcourseRevealEnd)
            {
            ...
            return new OpeningCameraStorySample(trainDestination, OpeningCameraStoryBeat.Train);
        }
