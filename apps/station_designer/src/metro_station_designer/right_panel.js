...
import { DebugLogPanel } from "./debug_log_panel.js?v=debug-log-1";
import { ExperimentPanel } from "./experiment_panel.js?v=control-plan-1";
...
export function RightPanel({
  ...
  return h("aside", { className: "inspector" }, [
    h("section", { key: "simulation", className: "section simulation-launch" }, [
      h("h2", { key: "title", className: "section__title" }, "4 仿真"),
      h(
        "button",
        {
          key: "run",
          className: "button button--primary simulation-launch__button",
          "data-testid": "run-simulation",
          onClick: runSimulation,
          disabled: simulating || compiling || simulationBlocked,
          title: simulationBlocked ? simulationBlockReason : "启动 Goal Graph + JuPedSim 预览",
        },
        simulating ? "仿真运行中…" : "4 开始仿真",
      ),
      simulationBlocked && simulationBlockReason
        ? h(
            "div",
            { key: "blocked", className: "simulation-result simulation-result--error" },
            simulationBlockReason,
          )
        : null,
    ...
    h("section", { key: "metrics", className: "section" }, [
      h("h2", { key: "title", className: "section__title" }, "站点校验"),
    ...
    h("section", { key: "selection", className: "section" }, [
      h("h2", { key: "title", className: "section__title" }, "当前选择"),
      h(SelectionDetails, {
        key: "details",
        onDemandFlowRateChange,
        onDemandFlowTargetChange,
    ...
    h("section", { key: "diag", className: "section" }, [
      h("h2", { key: "title", className: "section__title" }, "问题诊断"),
      h(DiagnosticList, { key: "diagnostics", issues }),
    ]),
    h(DebugLogPanel, { key: "debug-log" }),
  ]);
}
...
function SelectionDetails({
  ...
  if (data.demand_flow) {
    return h("div", null, [
      h(DetailRow, { key: "id", label: "flow", value: data.flow_id }),
      h(DetailRow, { key: "intent", label: "intent", value: data.intent }),
      h(DetailRow, { key: "source", label: "source", value: data.source_element_id }),
      data.intent === "transfer"
        ? h("label", { key: "target", className: "demand-target-field" }, [
            h("span", { key: "label" }, "target platform"),
            h(
              "select",
              {
                key: "select",
                "data-testid": "demand-transfer-target",
                value: data.target_element_id || "",
                onChange: (event) =>
                  onDemandFlowTargetChange(selectedNode.id, event.target.value),
              },
              [
                h("option", { key: "empty", value: "" }, "Select platform"),
                ...(platformOptions || [])
                  .filter((platform) => platform.id !== data.source_element_id)
                  .map((platform) =>
                    h(
                      "option",
                      { key: platform.id, value: platform.id },
                      `${platform.label} · ${platform.lineId} ${platform.direction}`,
                    ),
                  ),
              ],
            ),
        ...
        : null,
      h("label", { key: "rate", className: "demand-rate-field" }, [
        h("span", { key: "label" }, "rate"),
        h("input", {
          key: "input",
          type: "number",
          min: 0,
          ...
          "data-testid": "demand-flow-rate",
          onChange: (event) => onDemandFlowRateChange(selectedNode.id, event.target.value),
        }),
        h("span", { key: "unit" }, "p/h"),
      ]),
    ]);
