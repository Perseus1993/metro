...
def build_tracks() -> dict[str, object]:
    ...
    _assign_forward_track_headings(agents)
...
def _assign_forward_track_headings(agents: list[dict[str, object]]) -> None:
    """Orient each legacy replay point toward its following physical sample."""
    ...
    for agent in agents:
        ...
        _assign_forward_headings(
            points,
            snapshot_interval_seconds=float(SAMPLE_DT),
            movement_interval_seconds=float(SAMPLE_DT),
            x_units_per_meter=float(PX_PER_METER),
            y_units_per_meter=float(PX_PER_METER),
        )
