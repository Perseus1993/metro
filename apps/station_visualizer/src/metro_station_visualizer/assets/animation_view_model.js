(function initMetroAnimationViewModel(root, factory) {
  ...
  function buildAnimationViewModel(payload) {
    ...
    const replayPackage = objectOrNull(raw?.replay_package);
    const stationScene = objectOrNull(replayPackage?.station_scene);
    const assetManifest = objectOrNull(replayPackage?.asset_manifest);
    ...
    return {
      ...
      clearanceAudit,
      terminalEvents: arrayOr(
        simulationTrace?.terminal_events,
        arrayOr(raw?.terminal_events, []),
      ),
      evacuationMetrics: objectOr(
        derived?.evacuation_metrics,
        objectOr(raw?.evacuation_metrics, {}),
      ),
      duration,
      demoWindowSeconds: positiveNumber(raw?.demo_window_seconds, duration || 0),
      agents,
  ...
  function interpolateHeading(start, end, amount) {
    ...
    if (![from, to, t].every(Number.isFinite)) return from;
    const shortestDelta = Math.atan2(Math.sin(to - from), Math.cos(to - from));
    return from + shortestDelta * t;
  }
