...
def filter_vertical_transfers_for_passenger(
    ...
    expected = next(
        (
            facility_id
            for facility_id in facility_path
            if any(
                candidate.facility_id == facility_id
                and candidate.spec.entry_level_id == passenger.current_level_id
                for candidate in filtered
            )
        ),
        None,
    )
    return [item for item in filtered if item.facility_id == expected]
