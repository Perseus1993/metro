...
class QueueSpec:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "label": self.label,
            "service_direction": self.service_direction,
        }
...
# Queue compilation builds a bounded local navigation graph.  Real station
# templates use single-digit capacities; this generous ceiling prevents an
# untrusted design payload from turning compilation into an unbounded graph
# search while still supporting unusually large holding areas.
MAX_COMPILED_QUEUE_CAPACITY = 128
MIN_COMPILED_QUEUE_SPACING_M = 0.4
