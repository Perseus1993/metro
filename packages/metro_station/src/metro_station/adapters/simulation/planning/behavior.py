...
class PassengerBehaviorView(Protocol):
    intent: str
    state: str
...
def region_goal_for_passenger(passenger: PassengerBehaviorView) -> RegionGoal:
    ...
    if intent == AgentIntent.EVACUATE_STATION.value:
        return RegionGoal(
            intent=intent,
            origin_region="station_platform_population",
            destination_region="station_exterior_safe_zone",
            via_regions=("vertical_transfer", "exit_gate", "safe_zone"),
...
def behavior_status_for_passenger(passenger: PassengerBehaviorView) -> BehaviorStatus:
...
def _queue_mode_for_state(state: str, goal_kind: str | None) -> str | None:
    if state in QUEUE_STATES:
        return "enqueued"
    if goal_kind in {"queued", "queue_approach"}:
        return "targeting"
    return None


def _distance_to_target(passenger: PassengerBehaviorView) -> float | None:
