...
from .analysis.decisions import review_decisions
from .analysis.tracks import analyze_tracks
from .analysis.vertical import analyze_vertical
from metro_station.adapters.simulation.planning.plan import SERVICE_STATES
...
def _judge(
    *,
    ...
    if truth_checks.get("strict_clearance") and not truth_checks.get("fully_cleared"):
        blocker_codes = ", ".join(truth_checks.get("clearance_blocker_codes", []))
        issues.append(f"full clearance failed: {blocker_codes or 'unknown blocker'}")
    if stuck_agents > 0:
...
def _truth_checks(tracks_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    graph_debug = tracks_payload.get("graph_debug")
    if not isinstance(graph_debug, dict):
        graph_debug = {}
    graph_counts = graph_debug.get("counts", {})
    if not isinstance(graph_counts, dict):
        graph_counts = {}
    graph_checks = graph_debug.get("checks", {})
    if not isinstance(graph_checks, dict):
        graph_checks = {}
    clearance_blockers = graph_debug.get("blockers", [])
    if not isinstance(clearance_blockers, list):
        clearance_blockers = []

    spawned = _int(metrics.get("spawned_persons"))
    completed = _int(metrics.get("departed_persons"))
    if completed <= 0:
        completed = _int(metrics.get("boarded_persons")) + _int(
            metrics.get("exit_gate_served_persons")
        )
    remaining = _int(metrics.get("station_persons"))
    if remaining <= 0:
        remaining = len(final_passengers)
    if graph_debug:
        spawned = _int(graph_counts.get("spawned_persons"), spawned)
        remaining = _int(graph_counts.get("active_persons"), remaining)
        completed = max(0, spawned - remaining)
    ...
    truth_anomaly_count = (
        ...
        + len(graph_debug.get("missing_trajectory_ids", []))
        + len(graph_debug.get("incomplete_graph_ids", []))
        + len(graph_debug.get("duplicate_terminal_ids", []))
    )
    return {
        ...
        "strict_clearance": bool(graph_debug),
        "fully_cleared": bool(graph_debug.get("cleared", False)) if graph_debug else None,
        "clearance_checks": graph_checks,
        "clearance_blocker_codes": [
            str(blocker.get("code"))
            for blocker in clearance_blockers
            if isinstance(blocker, dict) and blocker.get("code")
        ],
        "missing_trajectory_ids": list(graph_debug.get("missing_trajectory_ids", [])),
        "incomplete_graph_ids": list(graph_debug.get("incomplete_graph_ids", [])),
        "duplicate_terminal_ids": list(graph_debug.get("duplicate_terminal_ids", [])),
    }
