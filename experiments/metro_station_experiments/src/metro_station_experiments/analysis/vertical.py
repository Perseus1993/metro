...
from metro_station.adapters.simulation.runtime.contracts import diagnostic_truth_payload
...
class VerticalServiceWindow:
    ...
    end_s: float
    track_ids: tuple[str, ...]
    line: tuple[tuple[float, float], ...] | None
...
def extract_vertical_service_windows(payload: dict[str, Any]) -> list[VerticalServiceWindow]:
    raw_events = payload.get("vertical_service_events")
    if not isinstance(raw_events, list):
        visual = payload.get("visualization_bundle")
        animations = visual.get("visual_facility_animations") if isinstance(visual, dict) else None
        raw_events = animations.get("vertical_service_events") if isinstance(animations, dict) else None
    if not isinstance(raw_events, list):
        trace = payload.get("simulation_trace")
        raw_events = trace.get("facility_events") if isinstance(trace, dict) else []

    windows: list[VerticalServiceWindow] = []
    for raw in raw_events:
        if not isinstance(raw, dict):
            continue
        kind = str(raw.get("facility_kind", raw.get("kind", "")))
        if kind not in {"elevator", "escalator", "stairs"}:
            continue
        try:
            start_s = float(raw.get("board_end_time") or raw.get("start_time", raw.get("start")))
            end_s = float(raw.get("arrive_time") or raw.get("end_time", raw.get("end")))
        except (TypeError, ValueError):
            continue
        if end_s <= start_s:
            continue
        raw_ids = raw.get("passenger_ids", raw.get("track_ids", []))
        track_ids = tuple(str(value) for value in raw_ids) if isinstance(raw_ids, list | tuple) else ()
        if not track_ids:
            continue
        start_canvas = raw.get("start_canvas")
        end_canvas = raw.get("end_canvas")
        line = None
        if (
            isinstance(start_canvas, list | tuple)
            and isinstance(end_canvas, list | tuple)
            and len(start_canvas) >= 2
            and len(end_canvas) >= 2
        ):
            try:
                line = (
                    (float(start_canvas[0]), float(start_canvas[1])),
                    (float(end_canvas[0]), float(end_canvas[1])),
                )
            except (TypeError, ValueError):
                line = None
        windows.append(
            VerticalServiceWindow(
                facility_id=str(raw.get("facility_id", raw.get("facility", "vertical_service"))),
                kind=kind,
...
def service_window_for_segment(
    windows: Sequence[VerticalServiceWindow],
    previous: Point,
    current: Point,
) -> VerticalServiceWindow | None:
    for window in windows:
        overlap = min(current.time, window.end_s) - max(previous.time, window.start_s)
        if overlap > 0.001:
            return window
    return None


def channel_for_service_window(
    window: VerticalServiceWindow,
    channels: Sequence[ConnectorChannel],
) -> ConnectorChannel | None:
    if window.line is None:
        return None
    candidates = [
        channel
        for channel in channels
        if channel.kind == window.kind
        and channel.direction in {window.direction, "both"}
    ]
    if not candidates:
        return ConnectorChannel(
            channel_id=window.facility_id,
            kind=window.kind,
    ...
    start, end = window.line[0], window.line[-1]

    def endpoint_distance(channel: ConnectorChannel) -> float:
        direct = math.dist(start, channel.line[0]) + math.dist(end, channel.line[-1])
        reverse = math.dist(start, channel.line[-1]) + math.dist(end, channel.line[0])
        return min(direct, reverse)

    return min(candidates, key=endpoint_distance)
...
def build_report(
    ...
    *,
    ...
    service_windows = extract_vertical_service_windows(payload)
    service_windows_by_track: dict[str, list[VerticalServiceWindow]] = {}
    for window in service_windows:
        for track_id in window.track_ids:
            service_windows_by_track.setdefault(track_id, []).append(window)
    ...
    for track in tracks:
        ...
        for previous, current in zip(track.points, track.points[1:], strict=False):
            ...
            service_window = service_window_for_segment(
                service_windows_by_track.get(track.track_id, ()),
                previous,
                current,
            )
            progress_line: tuple[tuple[float, float], ...] | None = None
            if service_window is not None:
                service_event_segment_count += 1
                # A truth event takes precedence over geometric proximity.  In
                # particular, a stairs ride must not be interpreted as reverse
                # motion on an adjacent up escalator.
                if service_window.kind not in config.kinds:
                    continue
                channel = channel_for_service_window(service_window, channels)
                if channel is None or service_window.line is None:
                    continue
                if channel.channel_id not in stats_by_channel:
                    stats_by_channel[channel.channel_id] = ConnectorStats(channel)
                hit = ChannelHit(channel=channel, distance_px=0.0, progress=0.0)
                progress_line = service_window.line
            else:
                hit = segment_hit(previous, current, channels, config)
                if hit is None:
                    continue
                channel = hit.channel
                progress_line = channel.line
            ...
            previous_progress = project_to_polyline(previous, progress_line).progress
            current_progress = project_to_polyline(current, progress_line).progress
            progress_delta = current_progress - previous_progress
            reverse_is_authoritative = service_window is not None or not service_windows
            if (
                reverse_is_authoritative
                and channel.direction in {"down", "up"}
                and progress_delta < -config.reverse_tolerance
            ):
    ...
    return {
        ...
        "summary": {
            ...
            "service_event_segments": service_event_segment_count,
            "reverse_basis": "facility_service_events" if service_windows else "geometry",
        ...
        "limits": [
            ...
            "Facility-service truth events take precedence during their time windows; geometric connector membership is used only outside those windows.",
        ],
    }
