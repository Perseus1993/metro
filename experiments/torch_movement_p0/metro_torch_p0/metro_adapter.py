"""Experimental Metro port that exercises injection without changing Metro code.

It deliberately supports only a rectangular walking envelope.  It is P0
evidence for the MovementBackend boundary, not a production replacement for
JuPedSim's polygonal geometry or trajectory evidence.
"""
...
class ExperimentalTorchMovementBackend(MovementBackend):
    ...
    def _ensure_world(self, model) -> None:
        ...
        self._walls = rectangular_walls(width=width, height=height, batch_size=1, device=self.device, dtype=torch.float32)
